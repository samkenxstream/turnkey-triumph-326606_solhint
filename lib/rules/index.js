...
const loadThirdPartyModule = require('../config/third-party-loader')

module.exports = function checkers(reporter, configVals, inputSrc, tokens, options) {
  ...
  const meta = {
    ...
    pluginSearchDir: options.pluginSearchDir
  }
...
function loadPlugin(pluginName, { reporter, config, inputSrc, fileName, pluginSearchDir }) {
  const plugins = loadThirdPartyModule(`solhint-plugin-${pluginName}`, pluginSearchDir)
