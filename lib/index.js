...
function processStr(inputStr, config = {}, options) {
  config = applyExtends(config, options)

  const ast = parseInput(inputStr)
  const tokens = parser.tokenize(inputStr, { loc: true })
  const reporter = new Reporter(tokens, config)
  const listener = new TreeListener(checkers(reporter, config, inputStr, tokens, options))
...
function processFile(file, config, options) {
  const report = processStr(
