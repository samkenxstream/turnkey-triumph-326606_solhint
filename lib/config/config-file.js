...
const loadThirdPartyModule = require('./third-party-loader')
...
const configGetter = (path, pluginSearchDir) => {
  ...
  const extensionConfig = loadThirdPartyModule(extensionPath, pluginSearchDir)

  return extensionConfig
}

const applyExtends = (config, options = {}, getter = configGetter) => {
  if (!config.extends) {
    return config
  }
  ...
  return config.extends.reduceRight((previousValue, parentPath) => {
    try {
      const extensionConfig = getter(parentPath, options.pluginSearchDir)
